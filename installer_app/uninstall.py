...
It will:
  ...
  5. Delete the install folder (Program Files\\XTPOS) — scheduled to run
...
def schedule_self_delete(directory, log):
    ...
    bat = os.path.join(tempfile.gettempdir(), "xtpos_cleanup.bat")
...
# --------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="XT POS uninstaller")
