...
All notable changes to the XT POS app are recorded here.
...
### Added
- `make_update.py` packages an update — a versioned `XTPOS-<version>.zip` of the
...
### Changed
- Rebranded from "Local POS" to **XT POS** (publisher **Xonal Tech**). The
  install folder is now `C:\Program Files\XTPOS`, the installer is
  `XTPOS-Setup.exe`, and the Add/Remove Programs entry, shortcuts, and exe file
  properties all read "XT POS". (The running app window title still comes from
...
- Packaging: `POS.exe`, `Update.exe` and `Uninstall.exe` are now single-file
  ...
  `setup/build/`, and leaves only the shippable `setup/XTPOS-Setup.exe`.
