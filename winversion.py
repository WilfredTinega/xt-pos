...
Every exe we ship (POS, Update, Uninstall, XTPOS-Setup) embeds the same
company branding, so File -> Properties -> Details reads "Xonal Tech" and the
...
"Xonal Tech" (see the optional signing step in build-setup.bat).
"""
import os
...
def version_info(exe_name, description):
    """Build the VSVersionInfo object branding one exe as Xonal Tech."""
    v = _version_tuple()
